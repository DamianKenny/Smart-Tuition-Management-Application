...
public class StudentQRManager {
    ...
    public StudentQRManager() {
        this.firestore = FirebaseFirestore.getInstance();
        this.realtimeDB = FirebaseDatabase.getInstance().getReference(QR_NODE);
    }

    public static StudentQRData parseQRData(String qrContent) {
        try {
            StudentQRData data = new StudentQRData();
            String[] parts = qrContent.split(";");
            for (String part : parts) {
                if (part.contains("=")) {
                    String[] keyVal = part.split("=");
                    if (keyVal[0].equalsIgnoreCase("studentId")) {
                        data.studentId = keyVal[1];
                    } else if (keyVal[0].equalsIgnoreCase("uuid")) {
                        data.uniqueId = keyVal[1];
                    }
                }
            }

            return (data.studentId != null && data.uniqueId != null) ? data : null;
        } catch (Exception e) {
            return null;
        }
    }
